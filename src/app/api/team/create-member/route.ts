...
export async function POST(req: Request) {
  const { name, email, password, role, color, hourly_rate_usd, salary_type, monthly_salary_usd } = await req.json()
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      salary_type: salary_type === 'monthly' ? 'monthly' : 'hourly',
      monthly_salary_usd: Number(monthly_salary_usd) || null,

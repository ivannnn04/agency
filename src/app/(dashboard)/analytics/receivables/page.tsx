...
export default function ReceivablesPage() {
  ...
  async function fetchData() {
    const { data } = await supabase
      .from('transactions')
      .select('amount, counterparty:counterparties(id, name), category:categories(name)')
      .eq('type', 'income')
      .eq('is_planned', false)

    if (!data) return

    // Receivables: income from counterparties that haven't been "settled" yet
    // For simplicity: show all income grouped by counterparty
  ...
  return (
    <div className="p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-800">Дебіторка <span className="text-teal-500">₴ {total.toLocaleString('uk-UA')}</span></h1>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              ...
              <th className="text-right py-3 px-4 text-gray-500 font-medium">Дебіторка</th>
          ...
          <tbody>
              ...
              <tr key={item.counterparty_id} className="border-b border-gray-50 hover:bg-gray-50/50">
                ...
                <td className="py-3 px-4 text-right">
                  <button
                    ...
                    className="bg-teal-50 text-teal-600 hover:bg-teal-100 px-3 py-1 rounded-lg text-xs font-medium transition-colors"
                  >
                    Погасити
                  </button>
                </td>
            ...
            {items.length === 0 && (
              <tr>
                <td colSpan={3} className="text-center py-12 text-gray-400">Немає дебіторської заборгованості</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {settleItem && (
        <SettleModal
          open={!!settleItem}
          title={`Погашення дебіторки для «${settleItem.counterparty_name}»`}

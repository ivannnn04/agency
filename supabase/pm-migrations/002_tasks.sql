...
create policy "Project members can view tasks" on public.pm_tasks for select using (
  project_id in (
    select project_id from public.pm_project_members where user_id = auth.uid()
  )
);
create policy "Project members can create tasks" on public.pm_tasks for insert with check (
  project_id in (
    select project_id from public.pm_project_members where user_id = auth.uid()
  )
);
create policy "Project members can update tasks" on public.pm_tasks for update using (
  project_id in (
    select project_id from public.pm_project_members where user_id = auth.uid()
  )
);
create policy "Project members can delete tasks" on public.pm_tasks for delete using (
  project_id in (
    select project_id from public.pm_project_members where user_id = auth.uid()
  )
);

create policy "Users can manage their own time logs" on public.pm_time_logs for all using (user_id = auth.uid());

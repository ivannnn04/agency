...
export default function CashFlowPage() {
  ...
  async function fetchData() {
    ...
    const months = Array.from({ length: 12 }, (_, i) => {
      ...
      const income = monthTxs.filter(t => t.type === 'income').reduce((s, t) => s + toEUR(t.amount, t.currency), 0)
      const expense = monthTxs.filter(t => t.type === 'expense').reduce((s, t) => s + toEUR(t.amount, t.currency), 0)
      return {
  ...
  function groupByCategory(txs: any[]) {
    ...
    txs.forEach(t => {
      ...
      map[name] = (map[name] ?? 0) + toEUR(t.amount, t.currency)
    })

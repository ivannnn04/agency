...
export default function LoginPage() {
  ...
  async function handleManagerLogin(e: React.FormEvent) {
    ...
    const res = await signIn('credentials', {
      email, password, redirect: false, callbackUrl: '/my-leads',
    })
    setLoading(false)
    if (res?.error) {
    ...
    <div className="min-h-screen bg-[#0f1117] flex items-center justify-center">
      <div className="bg-[#1a1d27] rounded-2xl p-10 w-full max-w-md text-center shadow-2xl">
        <div className="mb-8">
          ...
          <h1 className="text-2xl font-bold text-white">Gudrix Cowork Space</h1>
          <p className="text-gray-400 mt-2 text-sm">Управління бізнесом</p>
        </div>
        ...
        <div className="flex bg-[#0f1117] rounded-xl p-1 mb-6">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
              tab === 'admin' ? 'bg-teal-500 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Адмін
          </button>
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
              tab === 'manager' ? 'bg-teal-500 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Менеджер
          </button>
        ...
        {tab === 'admin' ? (
          <>
            ...
            <p className="text-gray-600 text-xs mt-4">
              Дизайнери входять на <span className="text-gray-400">/team/login</span>
            </p>
          </>
        ) : (

...
export default function Sidebar() {
  ...
  // Re-compute planned when rates load
  useEffect(() => {
      ...
      <aside className="w-[220px] min-w-[220px] bg-[#0f1117] text-white flex flex-col p-4 gap-4 overflow-y-auto border-r border-white/5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => setAddAccountOpen(true)}
              className="text-gray-500 hover:text-white transition-colors p-0.5 rounded" title="Додати рахунок">
              <Plus size={14} />
            </button>
          </div>
          <div className="flex flex-col gap-1">
        ...
        <div className="border-t border-white/10 pt-3 mt-auto">
          <div className="flex justify-between text-sm">
            <span className="text-gray-300 font-medium">Всього</span>
            <span className="text-white font-semibold">{fmtUSD(totalUSD)}</span>
          </div>
